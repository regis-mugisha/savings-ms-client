...
export const adminLogout = async (req, res) => {
  try {
    ...
    // This endpoint exists to allow the client to standardize the flow.
...
export const getUserById = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select(
      "fullName email balance deviceVerified deviceId createdAt"
    );
    if (!user) {
  ...
  try {
    ...
    if (user.pushToken) {
      try {
        ...
        console.log(
          `Failed to send device verification notification: ${e.message}`
        );
      }
    }
    return res.status(200).json({
      message: "User device verified",
      user: { _id: user._id, deviceVerified: user.deviceVerified },
    });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
...
export const dashboardStats = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ totalUsers, verifiedUsers, totalBalance, totalTransactions });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
